...
abstract class HardwareComponent {
    private znacka: string;
    private model: string;
    private cena: number;

    // Konstruktor: zkontroluje správnost dat a vytvoří komponentu
    constructor(znacka: string, model: string, cena: number) {
        if (!znacka.trim() || !model.trim()) throw new Error("Značka a model nesmí být prázdné.");
...
class CPU extends HardwareComponent {
    constructor(znacka: string, model: string, cena: number, readonly private cores: number, readonly private freq: number) {
        super(znacka, model, cena);
        if (cores <= 0 || freq <= 0) throw new Error("Parametry musí být kladné.");
    }
    public getInfo(): string { return `[CPU] ${this.getZakladniInfo()} | Jádra: ${this.cores}, Takt: ${this.freq} GHz`; }
}

// Grafická karta (přidává paměť VRAM)
class GPU extends HardwareComponent {
    constructor(znacka: string, model: string, cena: number, readonly private vram: number) {
        super(znacka, model, cena);
        if (vram <= 0) throw new Error("Parametr musí být kladný.");
    }
    public getInfo(): string { return `[GPU] ${this.getZakladniInfo()} | VRAM: ${this.vram} GB`; }
}

// Základní deska (přidává patici/socket)
class Motherboard extends HardwareComponent {
    constructor(znacka: string, model: string, cena: number, readonly private socket: string) {
        super(znacka, model, cena);
        if (!socket.trim()) throw new Error("Socket nesmí být prázdný.");
    }
    public getInfo(): string { return `[Základní deska] ${this.getZakladniInfo()} | Socket: ${this.socket}`; }
}

// Operační paměť (přidává kapacitu RAM)
class RAM extends HardwareComponent {
    constructor(znacka: string, model: string, cena: number, readonly private size: number) {
        super(znacka, model, cena);
        if (size <= 0) throw new Error("Parametr musí být kladný.");
    }
    public getInfo(): string { return `[RAM] ${this.getZakladniInfo()} | Kapacita: ${this.size} GB`; }
}

// Úložiště (přidává kapacitu disku)
class Storage extends HardwareComponent {
    constructor(znacka: string, model: string, cena: number, readonly private capacity: number) {
        super(znacka, model, cena);
        if (capacity <= 0) throw new Error("Parametr musí být kladný.");
    }
    public getInfo(): string { return `[Úložiště] ${this.getZakladniInfo()} | Kapacita: ${this.capacity} GB`; }
}

// Napájecí zdroj (přidává výkon ve Wattech)
class PSU extends HardwareComponent {
    constructor(znacka: string, model: string, cena: number, readonly private watts: number) {
        super(znacka, model, cena);
        if (watts <= 0) throw new Error("Parametr musí být kladný.");
    }
    public getInfo(): string { return `[Zdroj] ${this.getZakladniInfo()} | Výkon: ${this.watts} W`; }
}
...
class PCShop {
    public start(): void {
        ...
        for (const data of cenikKomponent) {
            ...
            if (data.typ === 'CPU') {
                const procesor = new CPU(data.znacka, data.model, data.cena, data.param1 as number, data.param2!);
                cart.add(procesor);
            } 
            else if (data.typ === 'GPU') {
                const grafika = new GPU(data.znacka, data.model, data.cena, data.param1 as number);
                cart.add(grafika);
            } 
            else if (data.typ === 'Motherboard') {
                const deska = new Motherboard(data.znacka, data.model, data.cena, data.param1 as string);
                cart.add(deska);
            } 
            else if (data.typ === 'RAM') {
                const pamet = new RAM(data.znacka, data.model, data.cena, data.param1 as number);
                cart.add(pamet);
            } 
            else if (data.typ === 'Storage') {
                const disk = new Storage(data.znacka, data.model, data.cena, data.param1 as number);
                cart.add(disk);
            } 
            else if (data.typ === 'PSU') {
                const zdroj = new PSU(data.znacka, data.model, data.cena, data.param1 as number);
                cart.add(zdroj);
            }
        }
